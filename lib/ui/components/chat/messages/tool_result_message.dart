...
class ToolResultMessage extends StatelessWidget {
  ...
  const ToolResultMessage({super.key, this.title = '', required this.content});

  @override
  Widget build(BuildContext context) {
    return FoldableMessage(
      title: title.isNotEmpty ? '$title 도구 실행 결과' : '도구 실행 결과',
      titleIcon: Icons.description_outlined,
      content: content,
      customContentBuilder:
          (contentList) => MarkdownBody(
            ...
            styleSheet: MarkdownStyleSheet(
              ...
              strong: MessageStyles.defaultTextStyle.copyWith(
                fontWeight: FontWeight.bold,
              ),
              em: MessageStyles.defaultTextStyle.copyWith(
                fontStyle: FontStyle.italic,
              ),
