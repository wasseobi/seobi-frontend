...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      // 생성된 샘플 메시지를 ChatProvider에 설정
      context.read<ChatProvider>().setMessages(_messages);
  ...
  void _generateSampleMessages() {
    ...
    final List<Map<String, dynamic>> conversations = [
      {
        ...
        'aiMsg':
            '안녕하세요! 저는 서비(Seobi)라고 합니다. 언어 학습, 문서 작업, 일상 질문 등 다양한 주제에 대해 도움을 드릴 수 있어요. 무엇을 도와드릴까요?',
        'messageType': MessageType.text,
      },
      {
        'userMsg': '오늘 날씨가 어때요?',
        'aiMsg':
            '죄송합니다. 저는 실시간 날씨 정보에 접근할 수 없습니다. 하지만 날씨에 대해 궁금하시다면 기상청 웹사이트나 날씨 앱을 확인해보시는 것을 추천드립니다.',
        'messageType': MessageType.text,
      },
      {
        'userMsg': '영어로 "안녕하세요"는 뭐예요?',
        'aiMsg':
            '"안녕하세요"는 영어로 "Hello"입니다. 비슷한 인사말로는 "Hi", "Hey", "Greetings" 등이 있습니다. 좀 더 정중하게 말하고 싶으시면 "Good morning/afternoon/evening"도 사용할 수 있습니다.',
        'messageType': MessageType.text,
      },
    ];
    ...
    final List<Map<String, dynamic>> cardMessages = [
      ...
      },
      {
        ...
        'messageType': MessageType.card,
        'card': {'title': '프로젝트 데드라인', 'time': '2025년 6월 5일', 'location': '회사'},
        'actions': [
          {'icon': '📋', 'text': '할 일 목록 보기'},
          {'icon': '✏️', 'text': '일정 수정하기'},
        ],
    ...
    while (messageCount < 20) {
      ...
      final messageTime = now.subtract(
        Duration(minutes: 10 * (messageCount ~/ 2)),
      );
      final formattedTime =
          '${messageTime.hour.toString().padLeft(2, '0')}:${messageTime.minute.toString().padLeft(2, '0')} 전송됨';
      ...
      final responseFormattedTime =
          '${responseTime.hour.toString().padLeft(2, '0')}:${responseTime.minute.toString().padLeft(2, '0')} 전송됨';
      ...
      // 액션 버튼이 있는 경우 추가
      if (messageData['messageType'] == MessageType.action &&
          messageData.containsKey('actions')) {
        aiMessage['actions'] = messageData['actions'];
      }

      // 카드가 있는 경우 추가
      if (messageData['messageType'] == MessageType.card &&
          messageData.containsKey('card')) {
